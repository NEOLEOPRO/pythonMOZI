def affine(p: str, k11: int, k12: int, k21: int, k22: int):
    ...
    ii = 0
    if gcd(k11) != 1:
        print("Warning: k11 первой пары не взаимно простой с 66, дешифрование с данным ключём не получится")
    if gcd(k21) != 1:
        print("Warning: k21 второй пары не взаимно простой с 66, дешифрование с данным ключём не получится")
    for i in p:
        if i in alf:
            if ii == 0:
                c += alf[(alf.find(i) * k11 + k12) % 66]
                ii += 1
                continue
            c += alf[(alf.find(i) * k21 + k22) % 66]
            k11, k21 = k21, k11 * k21
            k12, k22 = k22, k12 + k22
        else:
            c += i
...
def decode(p: str, k11: int, k12: int, k21: int, k22: int):
    ...
    ii = 0
    if gcd(k11) != 1:
        return "Error: k11 первой пары не взаимно простой с 66"
    if gcd(k21) != 1:
        return "Error: k21 второй пары не взаимно простой с 66"
    for i in p:
        if i in alf:
            for a in range(66):
                if (k11 * a) % 66 == 1:
            ...
            for b in range(66):
                if (k21 * b) % 66 == 1:
                    ...
                    break
            if ii == 0:
                c1 += alf[((alf.find(i) - k12) * k11_re) % 66]
                ii += 1
                continue
            c1 += alf[((alf.find(i) - k22) * k21_re) % 66]
            k11, k21 = k21, k11 * k21
            k12, k22 = k22, k12 + k22
        else:
            c1 += i
